...
int cat_f_a(struct parsed_command *cmd) {
    ...
    while (fat_value != 0xFFFF) {
        ...
        for (int i = 0; i < num_entries; i++) {
            ...
            // for (int j = 0; j < sizeof(dir_entry.name); j++) {
            //     fprintf(stderr, "%c", dir_entry.name[j]);
            // }
            // fprintf(stderr, "\n");
...
int main(int argc, char *argv[]) {
    ...
    while (getline(&input_line, &len, stdin) != -1) {
        ...
        int parseResult = parse_command(input_line, &cmd);
        if (parseResult < 0) {
            free(cmd);
            perror("Syntax error");
            exit(EXIT_FAILURE);
        ...
        if (cmd->commands[0] && cmd->commands[0][0]) {
            if (strcmp(cmd->commands[0][0], "mkfs") == 0) {
            ...
            } else if (strcmp(cmd->commands[0][0], "mount") == 0) {
                mount(cmd->commands[0][1]);
            } else if (strcmp(cmd->commands[0][0], "umount") == 0) {
                umount();
                ...
                fprintf(stderr, "Command not recognized found...\n");
            }
        }
